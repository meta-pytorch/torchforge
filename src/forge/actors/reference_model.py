...
import torch
from forge.actors.trainer import compute_logprobs, compute_sequence_logprobs
...
class ReferenceModel(ForgeActor):
    ...
    @endpoint
    async def generate(self, episode: "Episode") -> torch.Tensor:
        """
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        """
        Pure forward pass through the model itself
        """
        assert len(self.engine.model_parts) == 1, "PP not implemented yet"
        return self.engine.model_parts[0](input_ids)
