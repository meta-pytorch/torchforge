...
class ChatEnvironment(Environment):
    ...
    def step(self, action: ChatAction) -> ChatObservation:
        ...
        # Use the role from the message_to_action method if available
        # This is stored in the action's metadata by message_to_action
        role = action.metadata.get("original_role", "assistant")
        message: Message = {"role": role, "content": decoded_text}
    ...
    def message_to_action(self, message: Message) -> ChatAction:
        ...
        # Store the original role in the action's metadata
        # This will be used by the step method to preserve role information
        action = ChatAction(tokens=tokens)
        action.metadata["original_role"] = message["role"]
