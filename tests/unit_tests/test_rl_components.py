...
class TestIntegration:
    ...
    async def test_environment_policy_interaction(self):
        ...
        # Create mock rewarder as proper actor
        class MockRewarder(Actor):
            @endpoint
            async def compute_reward(self, state, action, next_state):
                return 1.0
        ...
        # Create action manually (simulating policy output)
        ...
        # Verify the reward was computed by the mock rewarder
        assert reward == 1.0
    ...
    def test_trajectory_creation(self):
        """Test creating a complete trajectory."""
        trajectory = Trajectory()

        # Add some data
        trajectory.states = [
            ToyObservation(step=0, data=torch.tensor([0.0]), text="Step 0, Value: 0.0"),
            ToyObservation(step=1, data=torch.tensor([1.0]), text="Step 1, Value: 1.0"),
        ]
        trajectory.actions = [
            ToyAction(data=torch.tensor([1.0])),
        ]
        trajectory.rewards = [2.0]
        trajectory.dones = [False]
        trajectory.infos = [{"step": 0}]

        assert len(trajectory.states) == 2
        assert len(trajectory.actions) == 1
        assert len(trajectory.rewards) == 1
        assert trajectory.rewards[0] == 2.0

    def test_config_validation(self):
        """Test that configuration objects work correctly."""
        config = CollectorConfig(max_collector_steps=10)
        assert config.max_collector_steps == 10

        # Test with different values
        config2 = CollectorConfig(max_collector_steps=5)
        assert config2.max_collector_steps == 5
