...
"""
Tests for service.py and router.py
"""
...
@pytest.mark.asyncio
async def test_service_stateless_batch_flush_max_size():
    """Batch should flush when max batch size is reached using default router (RoundRobin)."""
    # Create a service with 2 replicas and batching enabled
    service = await Counter.options(
        procs=1, num_replicas=2, max_batch_size=3, batch_max_wait_s=0.5
    ).as_service(2)
    try:
        # Enqueue batch_size + 1 requests to force a batch flush
        tasks = [asyncio.create_task(service._get_replica(None)) for _ in range(4)]
        results = await asyncio.gather(*tasks)

        # Check that results are only healthy replicas
        assert all(r.healthy for r in results)

        replica_indices = {r.idx for r in results[:3]}  # first batch of 3
        assert len(replica_indices) == 1  # all went to the same replica

        # Last request (4th) should be routed after timeout
        last_request = results[3]
        assert (
            last_request.idx not in replica_indices
        )  # Last request (4th) should be assigned to a different replica (RoundRobin)

        # After flush, batch queue should be empty
        assert service._batch_queue.qsize() == 0
    finally:
        await service.stop()
